var Datepicker = (function () {
 ...
 function setDate(datepicker, inputDates, options) {
  ...
  resetRemoveDate(newDates);
 }

 //Моя вставка - Д.К.
 function resetRemoveDate(newDates) {
  const resumeMainInfo = document.querySelector('.resume-main__info');
  const dataIncorrect = document.querySelector('.data-incorrect');
  const confirmDate = document.querySelector('.confirm-date');
  resumeMainInfo.classList.remove('calendar-open');
  let nowDate = new Date();
  if (+nowDate > newDates) {
   dataIncorrect.classList.remove('hide-block');
  } else {
   dataIncorrect.classList.add('hide-block');
   confirmDate.onclick = () => {
 ...
 function resetDate(nowDate, newDates) {
  const windowDeadlineLeft = document.querySelector('.window__deadline-left').querySelector('time');
  const windowDeadlineRight = document.querySelector('.window__deadline-right').querySelector('time');
  ...
  let restMins = restMin % (60);
  let strRestTime = `${restDays} дней ${restHours} часа ${restMins} минут`;
  windowDeadlineRight.innerText = strRestTime;
 }
 /**
